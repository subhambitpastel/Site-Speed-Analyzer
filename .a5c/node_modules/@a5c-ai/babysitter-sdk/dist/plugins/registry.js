...
exports.readPluginRegistry = readPluginRegistry;
exports.writePluginRegistry = writePluginRegistry;
exports.getPluginEntry = getPluginEntry;
exports.upsertPluginEntry = upsertPluginEntry;
exports.removePluginEntry = removePluginEntry;
exports.listPluginEntries = listPluginEntries;
...
function createEmptyRegistry() {
    return {
        schemaVersion: types_1.PLUGIN_REGISTRY_SCHEMA_VERSION,
 ...
 */
async function readPluginRegistry(scope, projectDir) {
    const registryPath = (0, paths_1.getRegistryPath)(scope, projectDir);
    try {
        const raw = await node_fs_1.promises.readFile(registryPath, "utf8");
 ...
 * @param projectDir - Required when scope is 'project'
 */
async function writePluginRegistry(registry, scope, projectDir) {
    const registryPath = (0, paths_1.getRegistryPath)(scope, projectDir);
    await (0, atomic_1.writeFileAtomic)(registryPath, JSON.stringify(registry, null, 2) + "\n");
}
/**
 ...
 */
function getPluginEntry(registry, pluginName) {
 ...
 */
function upsertPluginEntry(registry, entry) {
 ...
 */
function removePluginEntry(registry, pluginName) {
    if (!(pluginName in registry.plugins)) {
        throw new Error(`Plugin "${pluginName}" is not present in the registry`);
    }
 ...
 */
function listPluginEntries(registry) {
