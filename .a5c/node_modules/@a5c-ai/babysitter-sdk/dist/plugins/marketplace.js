...
exports.deriveMarketplaceName = deriveMarketplaceName;
exports.cloneMarketplace = cloneMarketplace;
exports.updateMarketplace = updateMarketplace;
exports.readMarketplaceManifest = readMarketplaceManifest;
 ...
 */
function deriveMarketplaceName(url) {
 ...
 */
async function cloneMarketplace(url, scope, projectDir, manifestPath, branch, force) {
    const name = deriveMarketplaceName(url);
    const marketplacesDir = (0, paths_1.getMarketplacesDir)(scope, projectDir);
    const targetDir = node_path_1.default.join(marketplacesDir, name);
    // Ensure parent directory exists
    await node_fs_1.promises.mkdir(marketplacesDir, { recursive: true });
    // Check if already cloned
    try {
        await node_fs_1.promises.access(targetDir);
        if (force) {
            await node_fs_1.promises.rm(targetDir, { recursive: true, force: true });
        }
        else {
            throw new Error(`Marketplace "${name}" already exists at ${targetDir}. Use --force to replace or plugin:update-marketplace to refresh.`);
        }
    }
    catch (error) {
        if (!(0, types_1.isNodeError)(error) || error.code !== "ENOENT") {
            throw error;
        }
    ...
    if (manifestPath) {
        await node_fs_1.promises.writeFile(node_path_1.default.join(targetDir, types_1.MANIFEST_PATH_FILENAME), manifestPath.replace(/\\/g, "/"), "utf8");
    }
    return targetDir;
}
/**
 ...
 */
async function updateMarketplace(marketplaceName, scope, projectDir, branch) {
    const dir = (0, paths_1.getMarketplaceDir)(marketplaceName, scope, projectDir);
    try {
 ...
 */
async function resolveManifestPath(dir) {
    // 1. Check for stored custom path
    try {
        const customPath = (await node_fs_1.promises.readFile(node_path_1.default.join(dir, types_1.MANIFEST_PATH_FILENAME), "utf8")).trim();
        const resolved = node_path_1.default.join(dir, customPath);
        await node_fs_1.promises.access(resolved);
        return resolved;
    }
    catch {
    ...
    const rootManifest = node_path_1.default.join(dir, types_1.MARKETPLACE_MANIFEST_FILENAME);
    try {
        await node_fs_1.promises.access(rootManifest);
        return rootManifest;
    }
    catch {
        // Not at root
    ...
    const claudePluginManifest = node_path_1.default.join(dir, ".claude-plugin", types_1.MARKETPLACE_MANIFEST_FILENAME);
    try {
        await node_fs_1.promises.access(claudePluginManifest);
        return claudePluginManifest;
    }
    catch {
        // Not found anywhere
    }
    throw new Error(`Marketplace manifest not found in ${dir}. Searched: ${types_1.MARKETPLACE_MANIFEST_FILENAME}, .claude-plugin/${types_1.MARKETPLACE_MANIFEST_FILENAME}, and ${types_1.MANIFEST_PATH_FILENAME}. Is this a valid marketplace?`);
}
/**
 ...
 */
async function readManifestWithPath(marketplaceName, scope, projectDir) {
    const dir = (0, paths_1.getMarketplaceDir)(marketplaceName, scope, projectDir);
 ...
 * @param projectDir - Required when scope is 'project'
 */
async function readMarketplaceManifest(marketplaceName, scope, projectDir) {
 ...
 * @param projectDir - Required when scope is 'project'
 */
async function listMarketplacePlugins(marketplaceName, scope, projectDir) {
 ...
 * @param projectDir - Required when scope is 'project'
 */
async function resolvePluginPackagePath(marketplaceName, pluginName, scope, projectDir) {
    ...
    return node_path_1.default.join(manifestDir, entry.packagePath);
}
/**
 ...
 */
async function listMarketplaces(scope, projectDir) {
    const dir = (0, paths_1.getMarketplacesDir)(scope, projectDir);
    try {
