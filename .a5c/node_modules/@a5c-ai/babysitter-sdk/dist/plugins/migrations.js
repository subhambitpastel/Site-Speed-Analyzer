...
exports.buildMigrationGraph = buildMigrationGraph;
exports.findMigrationPath = findMigrationPath;
exports.resolveMigrationChain = resolveMigrationChain;
 ...
 */
const MIGRATION_FILENAME_REGEX = /^([a-zA-Z0-9._-]+)_to_([a-zA-Z0-9._-]+)\.(md|js)$/;
/**
...
async function listMigrations(migrationsDir) {
    let entries;
    try {
        entries = await node_fs_1.promises.readdir(migrationsDir);
    }
    catch (error) {
 ...
 */
function buildMigrationGraph(migrations) {
    const graph = new Map();
 ...
 */
async function resolveMigrationChain(packageDir, fromVersion, toVersion) {
    const migrationsDir = node_path_1.default.join(packageDir, "migrations");
    const allMigrations = await listMigrations(migrationsDir);
    const migrationPath = findMigrationPath(allMigrations, fromVersion, toVersion);
    if (!migrationPath) {
        return undefined;
    }
    const results = [];
    for (const descriptor of migrationPath) {
        const filePath = node_path_1.default.join(migrationsDir, descriptor.file);
