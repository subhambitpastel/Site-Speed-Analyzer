...
async function handleHookLog(args) {
    ...
    try {
        const resolvedLogFile = (0, resolveInputPath_1.resolveInputPath)(logFile);
        const logDir = path.dirname(resolvedLogFile);
        await node_fs_1.promises.mkdir(logDir, { recursive: true });
        await node_fs_1.promises.appendFile(resolvedLogFile, logLine + "\n", "utf8");
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
    ...
    if (json) {
        console.log(JSON.stringify({
            hookType,
            eventLabel,
            logFile: (0, resolveInputPath_1.resolveInputPath)(logFile),
            logLine,
        }));
    }
    else {
