...
exports.handlePluginAddMarketplace = handlePluginAddMarketplace;
exports.handlePluginUpdateMarketplace = handlePluginUpdateMarketplace;
exports.handlePluginListPlugins = handlePluginListPlugins;
exports.handlePluginInstall = handlePluginInstall;
exports.handlePluginUninstall = handlePluginUninstall;
exports.handlePluginUpdate = handlePluginUpdate;
exports.handlePluginConfigure = handlePluginConfigure;
exports.handlePluginListInstalled = handlePluginListInstalled;
exports.handlePluginUpdateRegistry = handlePluginUpdateRegistry;
exports.handlePluginRemoveFromRegistry = handlePluginRemoveFromRegistry;
const node_fs_1 = require("node:fs");
const node_path_1 = __importDefault(require("node:path"));
const marketplace_1 = require("../../plugins/marketplace");
const packageReader_1 = require("../../plugins/packageReader");
const migrations_1 = require("../../plugins/migrations");
...
function requireScope(scope, command, json) {
 ...
 */
async function autoResolveMarketplace(scope, projectDir) {
    const marketplaces = await (0, marketplace_1.listMarketplaces)(scope, projectDir);
 ...
 */
async function handlePluginAddMarketplace(args) {
    ...
    try {
        const targetDir = await (0, marketplace_1.cloneMarketplace)(marketplaceUrl, scope, projectDir, marketplacePath, marketplaceBranch, force);
        if (json) {
 ...
 */
async function handlePluginUpdateMarketplace(args) {
    ...
    const projectDir = scope === "project" ? process.cwd() : undefined;
    try {
        await (0, marketplace_1.updateMarketplace)(marketplaceName, scope, projectDir, marketplaceBranch);
        if (json) {
            console.log(JSON.stringify({
                success: true,
 ...
 * Requires: marketplaceName, scope
 */
async function handlePluginListPlugins(args) {
    ...
    if (!name)
        return 1;
    try {
        const plugins = await (0, marketplace_1.listMarketplacePlugins)(marketplaceName, scope, projectDir);
        if (json) {
            console.log(JSON.stringify({
                marketplace: marketplaceName,
                scope,
            ...
            if (plugins.length === 0) {
                ...
                const versionWidth = Math.max(...plugins.map((p) => p.latestVersion.length), 7);
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (json) {
            console.log(JSON.stringify({ error: "list_failed", message }));
        }
        else {
...
async function handlePluginInstall(args) {
    ...
    if (!plugin)
        return 1;
    if (!requireScope(scope, "plugin:install", json))
        return 1;
    const projectDir = scope === "project" ? process.cwd() : undefined;
    // Auto-resolve marketplace when not provided and only one exists
    if (!marketplaceName) {
    ...
    const marketplace = requireArg(marketplaceName, "--marketplace-name", "plugin:install", json);
    if (!marketplace)
        return 1;
    try {
        ...
        if (!version) {
            const manifest = await (0, marketplace_1.readMarketplaceManifest)(marketplaceName, scope, projectDir);
        ...
        const packagePath = await (0, marketplace_1.resolvePluginPackagePath)(marketplaceName, pluginName, scope, projectDir);
        // Read install instructions
        const instructions = await (0, packageReader_1.readInstallInstructions)(packagePath);
        // Check for install-process.js
        const processFilePath = node_path_1.default.join(packagePath, "install-process.js");
        let processFile = null;
        try {
            await node_fs_1.promises.access(processFilePath);
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (json) {
            console.log(JSON.stringify({ error: "install_failed", message }));
        }
        else {
...
async function handlePluginUninstall(args) {
    ...
    try {
        ...
        const registry = await (0, registry_1.readPluginRegistry)(scope, projectDir);
        const entry = (0, registry_1.getPluginEntry)(registry, pluginName);
        if (!entry) {
        ...
        const packagePath = await (0, marketplace_1.resolvePluginPackagePath)(marketplace, pluginName, scope, projectDir);
        // Read uninstall instructions
        const instructions = await (0, packageReader_1.readUninstallInstructions)(packagePath);
        // Check for uninstall-process.js
        const processFilePath = node_path_1.default.join(packagePath, "uninstall-process.js");
        let processFile = null;
        try {
            await node_fs_1.promises.access(processFilePath);
            processFile = processFilePath;
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (json) {
            console.log(JSON.stringify({ error: "uninstall_failed", message }));
        }
        else {
...
async function handlePluginUpdate(args) {
    const { pluginName, marketplaceBranch, scope, pluginVersion, json } = args;
    let { marketplaceName } = args;
    const plugin = requireArg(pluginName, "--plugin-name", "plugin:update", json);
    if (!plugin)
        return 1;
    if (!requireScope(scope, "plugin:update", json))
        return 1;
    const projectDir = scope === "project" ? process.cwd() : undefined;
    // Auto-resolve marketplace when not provided and only one exists
    if (!marketplaceName) {
    ...
    const marketplace = requireArg(marketplaceName, "--marketplace-name", "plugin:update", json);
    if (!marketplace)
        return 1;
    try {
        ...
        const chain = await (0, migrations_1.resolveMigrationChain)(packagePath, installedVersion, targetVersion);
        if (!chain) {
            throw new Error(`No migration path found from version "${installedVersion}" to "${targetVersion}" for plugin "${pluginName}"`);
        }
        const migrations = chain.map(({ descriptor, content }) => ({
            ...
            processFile: descriptor.type === "js"
                ? node_path_1.default.join(packagePath, "migrations", descriptor.file)
                : null,
        }));
        const result = {
            plugin: pluginName,
...
async function handlePluginConfigure(args) {
    const { pluginName, scope, json } = args;
    let { marketplaceName } = args;
    const plugin = requireArg(pluginName, "--plugin-name", "plugin:configure", json);
    if (!plugin)
        return 1;
    if (!requireScope(scope, "plugin:configure", json))
        return 1;
    const projectDir = scope === "project" ? process.cwd() : undefined;
    // Auto-resolve marketplace when not provided and only one exists
    if (!marketplaceName) {
    ...
    const marketplace = requireArg(marketplaceName, "--marketplace-name", "plugin:configure", json);
    if (!marketplace)
        return 1;
    try {
        ...
        const instructions = await (0, packageReader_1.readConfigureInstructions)(packagePath);
        // Check for configure-process.js
        const processFilePath = node_path_1.default.join(packagePath, "configure-process.js");
        let processFile = null;
        try {
            await node_fs_1.promises.access(processFilePath);
            processFile = processFilePath;
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (json) {
            console.log(JSON.stringify({ error: "configure_failed", message }));
        }
        else {
...
async function handlePluginListInstalled(args) {
    const { scope, json } = args;
    ...
    try {
        const registry = await (0, registry_1.readPluginRegistry)(scope, projectDir);
        const entries = (0, registry_1.listPluginEntries)(registry);
        if (json) {
            ...
            if (entries.length === 0) {
                ...
                const versionWidth = Math.max(...entries.map((e) => e.version.length), 7);
                const marketplaceWidth = Math.max(...entries.map((e) => e.marketplace.length), 11);
...
async function handlePluginUpdateRegistry(args) {
    ...
    try {
        ...
        let packagePath = "";
        try {
            packagePath = await (0, marketplace_1.resolvePluginPackagePath)(marketplaceName, pluginName, scope, projectDir);
        }
        catch {
            // Marketplace may not exist or plugin may not be listed yet — that's OK
        }
        const now = new Date().toISOString();
        const existingEntry = (0, registry_1.getPluginEntry)(registry, pluginName);
        const entry = {
            name: pluginName,
        ...
        const updatedRegistry = (0, registry_1.upsertPluginEntry)(registry, entry);
        await (0, registry_1.writePluginRegistry)(updatedRegistry, scope, projectDir);
        if (json) {
...
async function handlePluginRemoveFromRegistry(args) {
    const { pluginName, scope, json } = args;
    ...
    try {
        const registry = await (0, registry_1.readPluginRegistry)(scope, projectDir);
        const updatedRegistry = (0, registry_1.removePluginEntry)(registry, pluginName);
        await (0, registry_1.writePluginRegistry)(updatedRegistry, scope, projectDir);
        if (json) {
